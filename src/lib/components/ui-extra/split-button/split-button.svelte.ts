...
type SplitButtonRootProps = WritableBoxedValues<{
  ...
  ReadableBoxedValues<{
    ...
    onClickPromise:
      | ((event: SplitButtonClickEvent) => Promise<void>)
      | undefined;
...
type SplitButtonActionStateProps = ReadableBoxedValues<{
  value: string;
  onclick:
    | HTMLButtonAttributes['onclick']
    | HTMLAnchorAttributes['onclick']
    | undefined;
}>;
...
class SplitButtonActionState {
  ...
  isActive = $derived.by(
    () => this.opts.value.current === this.rootState.opts.value.current
  );
...
export function useSplitButtonSelectAction(
  props: SplitButtonSelectActionStateProps
) {
