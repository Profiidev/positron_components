<script lang="ts">
  ...
  import { useFileDropZoneTextarea } from '$lib/components/ui-extra/file-drop-zone/file-drop-zone.svelte.js';
  import { box, mergeProps } from 'svelte-toolbelt';
  import type { WithChild } from 'bits-ui';

  type Props = HTMLAttributes<HTMLTextAreaElement>;

  let { onpaste, ondragover, ondrop, child, ...rest }: WithChild & Props =
    $props();
  ...
  const mergedProps = $derived(
    mergeProps(fileDropZoneTextareaState.props, rest)
  );
</script>
