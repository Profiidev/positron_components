...
class StepperRootState {
  ...
  selectStep(stepId: string) {
    this.opts.step.current =
      this.steps.findIndex((step) => step.id === stepId) + 1;
  }
...
class StepperItemTriggerState {
  ...
  _onkeydown(
    ...
    switch (e.key) {
      case 'ArrowRight':
        if (this.itemState.navState.opts.orientation.current === 'vertical')
          return;
      ...
      case 'ArrowLeft':
        ...
        break;
      case 'ArrowDown':
        if (this.itemState.navState.opts.orientation.current === 'horizontal')
          return;
...
export function useStepperItem(props: StepperItemProps) {
  return StepperItemCtx.set(
    new StepperItemState(props, StepperNavCtx.get(), StepperCtx.get())
  );
}
